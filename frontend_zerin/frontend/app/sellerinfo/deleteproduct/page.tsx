...
export default function DeleteProduct() {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`http://localhost:3002/product/${id}`, { withCredentials: true });
  ...
  const handleLogout = async () => {
    try {
      await axios.post("http://localhost:3002/seller/logout", {}, { withCredentials: true });
      router.push("/login");
    } catch (err) {
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {products.length > 0 ? (
                  ...
                  <tr>
                    <td className="p-3 border text-center text-black" colSpan={5}>
                      No products
                    </td>
                  </tr>
                )}
